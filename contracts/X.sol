...
contract X {
  function recover(bytes memory data, bytes memory signature) external pure returns (address) {
    return ECDSA.recover(MessageHashUtils.toEthSignedMessageHash(keccak256(data)), signature);
  }
